...
const repo=JSON.parse(await read("ROCKSOUL-REPO.json"))
const changelog=await read("CHANGELOG.md")
const releaseAudit=await read("scripts/release-audit.mjs")
...
if(await exists("yarn.lock")) lockfiles.push("yarn.lock")
...
if(!changelog.includes(`## v${pkg.version}`)) warnings.push(`CHANGELOG has no v${pkg.version} heading yet; publish gate must require one before a release tag exists`)
...
console.log(`Strict repository audit passed: ${pkg.name}@${pkg.version}`)
