  ...
  build: {
    lib: {
      entry: {
        ...
        ...Object.fromEntries(readdirSync(resolve(import.meta.dirname, "src/components/templates"))
          .filter(name => /\.(ts|tsx)$/.test(name))
          .map(name => [`components/templates/${name.replace(/\.tsx?$/, "")}`, resolve(import.meta.dirname, "src/components/templates", name)])),
      },
      formats: ["es"],
